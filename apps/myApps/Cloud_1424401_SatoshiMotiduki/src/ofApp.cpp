   ...
   /* for (int i=0; i<camWidth; i++) {
    ...
    for (int x=0; x<camWidth; x++) {
        ...
        for (int y=0; y<camHeight; y++) {
            ...
            if(y-1 >= 0 ){
                //mathNo += (float)convertImagePixels[(y-1)*camWidth+(x-1)]*convertNo[0];
                mathNo += (float)convertImagePixels[(y-1)*camWidth+x]*convertNo[1];
                //mathNo += (float)convertImagePixels[(y-1)*camWidth+(x+1)]*convertNo[2];
            }
            
            if(x-1 >= 0 && x+1 < camWidth){
                mathNo += (float)convertImagePixels[y*camWidth+(x-1)]*convertNo[3];
                //mathNo += (float)convertImagePixels[y*camWidth+x]*convertNo[4];
                mathNo += (float)convertImagePixels[y*camWidth+(x+1)]*convertNo[5];
            }
            
            if(y+1 < camHeight){
                //mathNo += (float)convertImagePixels[(y+1)*camWidth+(x-1)]*convertNo[6];
                mathNo += (float)convertImagePixels[(y+1)*camWidth+x]*convertNo[7];
                //mathNo += (float)convertImagePixels[(y+1)*camWidth+(x+1)]*convertNo[8];
            }
            ...
            convertStock[y*camWidth+x] = (char)mathNo;
    ...
    for (int i=0; i<camWidth; i++) {
        ...
        for (int j=0; j<camHeight; j++) {
            ...
            float _stock = 0;
            _stock = (float)convertStock[j*camWidth+i]*1.1;
